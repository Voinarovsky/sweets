...
class TradingBotApp:
    ...
    def win_tkinter(self):
        if self.win is None:  # Проверяем, что окно еще не создано
            ...
            self.win = tk.Tk()
            photo = tk.PhotoImage(file='free-icon-stocks-831273.png')
            ...
            tk.Label(self.win, text='Slow SMA:', bg="#485b7a", fg="#160150").grid(row=1, column=2, sticky='e')
            Fastsma = tk.Entry(self.win)
            Fastsma.grid(row=1, column=3, padx=20, pady=30)

            tk.Label(self.win, text='Fast SMA:', bg="#485b7a", fg="#160150").grid(row=2, column=2, sticky='e')
            slow_smm = tk.Entry(self.win)  # Используем уникальное имя переменной
            slow_smm.grid(row=2, column=3, padx=20, pady=30)

            tk.Label(self.win, text='%', font="Sans 8", bg="white", fg="#160150").grid(row=3, column=1,sticky='e')
            tk.Label(self.win, text='Торговая пара:', bg="#485b7a", fg="#160150").grid(row=3, column=0, sticky='e')
            inst = tk.Entry(self.win)  # Используем уникальное имя переменной
            inst.grid(row=3, column=1, padx=20, pady=30)

            tk.Label(self.win, text='Риск:', bg="#485b7a", fg="#160150").grid(row=2, column=0, sticky='e')
            risk = tk.Entry(self.win)  # Используем уникальное имя переменной
            risk.grid(row=2, column=1, padx=20, pady=30)
